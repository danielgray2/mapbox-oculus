...
public class TenGlyph : IAbstractGraph
{
    public TenGlyph(TenGlyphModel model)
    {
        this.model = model;
    ...
    {
        if(!(model is TenGlyphModel tGModel))
        {
            throw new ArgumentException("Model must be of type TenGlyphModel");
        }
